// lib/tennisMatches.js
//
// Logica delle partite di tennis del giorno: probabilità dal modello
// (tennisModel.js + Elo + H2H + forma recente) e confronto con le quote reali
// (OddsPapi) per il value bet. Spostata qui da app/api/tennis/matches/route.js
// il 23/09/2026 perché la usa anche la cron daily-digest direttamente.
//
// NOTA SULLA FONTE QUOTE: inizialmente usava The Odds API, ma copriva solo 1-2
// tornei di tennis alla volta. OddsPapi (oddspapi.io) ha copertura molto più
// ampia con piano gratuito limitato a 250 richieste/mese: per questo c'è una
// cache di 20 minuti su Supabase (tennis_odds_cache).

import { createClient } from "@supabase/supabase-js";
import { matchProb } from "./tennisModel";
...
// Esegue fn su ogni elemento con al massimo `limit` elaborazioni in parallelo, mantenendo l'ordine.
...
const MATCH_CONCURRENCY = 6; // match elaborati in parallelo (ognuno fa ~12 query su Supabase)

/**
 * Partite di tennis del giorno con probabilità del modello e quote.
 * Usata dalla route /api/tennis/matches e, direttamente, dalla cron daily-digest
 * (così la cron non deve chiamare l'endpoint via internet).
 * Ritorna { matches, fromCache } oppure { error, details?, matches: [] }.
 * Con debug = true ritorna le fixture/quote grezze.
 */
export async function getTennisMatches(date, { debug = false } = {}) {
  if (!ODDSPAPI_KEY) {
    return { error: "ODDSPAPI_KEY non configurata", matches: [] };
  }
  const isDebug = !!debug;
  ...
  if (!rawMatches) {
    try {
      ...
      return { error: "Impossibile contattare OddsPapi", details: String(e), matches: [] };
    }
  }

  if (isDebug) {
    return { fromCache, count: rawMatches.length, oddsDebug, rawMatches };
  }
  ...
  // Prima i match venivano elaborati uno alla volta (43 match × ~12 query = oltre 20 secondi).
  // Ora in parallelo a gruppi di MATCH_CONCURRENCY, stesso risultato e stesso ordine.
  const results = await mapLimit(rawMatches, MATCH_CONCURRENCY, async (m) => {
  ...
  return { matches: results, fromCache };
}
