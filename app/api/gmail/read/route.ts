...
async function analizzaPromozioni(mail: any[], nomeCliente: string) {
  ...
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Sei un assistente che analizza email. Rispondi SEMPRE e SOLO con un JSON array valido, senza markdown, senza testo prima o dopo.'
          },
          {
            role: 'user',
    ...
    if (!data.choices?.[0]?.message?.content) {
      console.error('Groq no content:', JSON.stringify(data))
      return []
    }
