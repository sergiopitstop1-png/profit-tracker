...
const HISTORY_TABLE = "prop_hedge_operations";
...
function opposite(d){ return d==="BUY"?"SELL":d==="SELL"?"BUY":"WAIT"; }
function checkEnv(){
  if(!SUPABASE_URL || !SERVICE_KEY) throw new Error("Supabase non configurato");
}
...
async function rest(path,options={}){
  ...
  if(!r.ok) throw new Error(typeof j==="string"?j:(j?.message||j?.error||`HTTP ${r.status}`));
  return j;
}
...
function lastBar(row){
  const bars=Array.isArray(row?.m15)?row.m15:[];
  if(!bars.length) return null;
  const b=bars[bars.length-1];
  return {
    t:n(b?.t,null), o:n(b?.o,null), h:n(b?.h,null), l:n(b?.l,null), c:n(b?.c,null)
  };
}
function propHit(active,bar){
  if(!bar || !["BUY","SELL"].includes(active?.direction)) return null;
  const tp=n(active.propTP,NaN), sl=n(active.propSL,NaN);
  if(!Number.isFinite(tp)||!Number.isFinite(sl)) return null;
  const buy=active.direction==="BUY";
  const tpHit=buy ? bar.h>=tp : bar.l<=tp;
  const slHit=buy ? bar.l<=sl : bar.h>=sl;
  if(tpHit && slHit) return "AMBIGUOUS";
  if(tpHit) return "TP";
  if(slHit) return "SL";
  return null;
}
function propPL(active,exit){
  const sign=active.direction==="BUY"?1:-1;
  return (exit-n(active.entry))*n(active.contract,100)*n(active.propLots)*sign*n(active.quoteToUsd,1);
}
function fmt(v,d=2){ return Number(v).toFixed(d); }

async function patchChallenge(row,state){
  const q=`${ACTIVE_TABLE}?user_id=eq.${encodeURIComponent(row.user_id)}&challenge_id=eq.${encodeURIComponent(row.challenge_id)}`;
  await rest(q,{
    method:"PATCH",
...
async function insertHistory(row,state,active,exit,event,pl){
  const brokerDirection=active.brokerDirection || opposite(active.direction);
  const payload={
    user_id:row.user_id,
    opened_at:active.placedAt,
    closed_at:new Date().toISOString(),
    prop_name:state.name || row.prop_name || "Prop",
    asset:active.asset,
    prop_direction:active.direction,
    broker_direction:brokerDirection,
    status:"closed",
    account_size:n(state.accountSize,null),
    prop_balance_start:n(active.propBalanceStart,null),
    prop_balance_end:n(active.propBalanceStart)+pl,
    broker_balance_start:n(active.brokerBalanceStart,null),
    broker_balance_end:null,
    prop_cost:n(state.propCost,null),
    final_profit_target:n(state.finalProfitTarget,null),
    risk_usd:n(state.risk,null),
    sl_distance:Math.abs(n(active.entry)-n(active.propSL)),
    tp_prop_usd:n(state.tpProp,null),
    dd_max_pct:n(state.ddMax,null),
    max_margin_pct:n(state.maxMarginPct,null),
    leverage:n(state.leverage,null),
    broker_exposure_start:n(state.brokerExposure,null),
    entry_price:n(active.entry,null),
    exit_price:exit,
    prop_lots:n(active.propLots,null),
    broker_lots:n(active.brokerLots,null),
    prop_tp_price:n(active.propTP,null),
    prop_sl_price:n(active.propSL,null),
    broker_tp_price:n(active.brokerTP,null),
    broker_sl_price:n(active.brokerSL,null),
    broker_max_loss:n(active.maxBrokerLossAtEntry,null),
    prop_pl:pl,
    broker_pl:null,
    combined_pl:pl,
    used_manual_prop_pl:false,
    used_manual_broker_pl:false,
    metadata:{
      automatic_trigger:event,
      execution_source:active.executionSource || "unknown",
      telegram_watchdog:true,
      balance_status:"CALCOLATO_NON_CONFERMATO"
    }
  };
  await rest(HISTORY_TABLE,{
    method:"POST",
    headers:{Prefer:"return=minimal"},
...
async function processChallenge(row){
  ...
  if(!active || active.monitoringActive===false) return {challenge:row.challenge_id,status:"NO_ACTIVE"};

  const symbol=active.asset || state.asset || "XAUUSD";
  const [sig,fd]=await Promise.all([
    latestSignal(symbol,active.placedAt),
    feed(symbol)
  ]);
  const bar=lastBar(fd);
  const mon={...(active.telegram||{})};
  const alerts=[];

  const initial=String(active.initialForecast || opposite(active.direction) || "WAIT").toUpperCase();
  const current=String(sig?.forecast_direction || "WAIT").toUpperCase();
  const signalKey=String(sig?.signal_m15_time || sig?.id || "");

  // ---- Forecast watchdog
  if(signalKey && signalKey!==mon.lastSignalTime){
    mon.lastSignalTime=signalKey;

    if(current==="WAIT"){
      mon.waitCount=n(mon.waitCount)+1;

      if(mon.waitCount===1 && !mon.firstWaitSent){
        await telegram(
        ...
        mon.firstWaitSent=true; alerts.push("FIRST_WAIT");
      } else if(mon.waitCount>=2 && !mon.secondWaitSent){
        await telegram(
        ...
        mon.secondWaitSent=true; alerts.push("SECOND_WAIT");
      }
    } else if(["BUY","SELL"].includes(current)){
      if(current===initial){
        mon.waitCount=0;
        mon.firstWaitSent=false;
        mon.secondWaitSent=false;
        mon.reversalSent=false;
      } else if(!mon.reversalSent){
        await telegram(
          `🚨 <b>CONTROTENDENZA CONFERMATA</b>\n`+
          `${state.name || row.prop_name} · ${symbol}\n`+
          `Forecast apertura: <b>${initial}</b>\n`+
          `Forecast corrente: <b>${current}</b>\n`+
          `${mon.waitCount>0 ? `WAIT precedenti: <b>${mon.waitCount}</b>\n` : ""}`+
          `🔴 Il trend si è girato contro lo scenario iniziale.\n`+
          `<b>VALUTA CHIUSURA IMMEDIATA.</b>`
        );
        mon.reversalSent=true; alerts.push("REVERSAL");
      }
    }
  }

  // ---- M15 body countertrend 4 / 8 USD
  if(bar && Number.isFinite(bar.o) && Number.isFinite(bar.c)){
    const body=bar.c-bar.o;
    const against = initial==="SELL" ? body : initial==="BUY" ? -body : 0;
    const barKey=String(bar.t);

    if(against>=8 && mon.m15Warn8SentFor!==barKey){
      await telegram(
      ...
      alerts.push("M15_8");
    } else if(against>=4 && mon.m15Warn4SentFor!==barKey){
      await telegram(
  ...
  // ---- TP / SL
  const hit=propHit(active,bar);
  if(hit==="AMBIGUOUS"){
    if(mon.ambiguousSentFor!==String(bar?.t)){
      await telegram(
        `⚠️ <b>TP E SL ATTRAVERSATI NELLA STESSA M15</b>\n`+
        `${state.name || row.prop_name} · ${symbol}\n`+
        `Ordine intrabar non determinabile. Verifica manualmente.`
      );
      mon.ambiguousSentFor=String(bar?.t);
      alerts.push("AMBIGUOUS");
    }
  } else if(hit==="TP" || hit==="SL"){
    const exit = hit==="TP" ? n(active.propTP) : n(active.propSL);
    const pl = propPL(active,exit);
    const newBal=n(active.propBalanceStart)+pl;

    if((hit==="TP" && !mon.tpSent) || (hit==="SL" && !mon.slSent)){
      await telegram(
      ...
      if(hit==="TP") mon.tpSent=true; else mon.slSent=true;
      alerts.push(hit);
    }

    await insertHistory(row,state,active,exit,hit,pl);

    const nextState={
      ...state,
      accountBalance:String(Number(newBal.toFixed(2))),
      active:null,
      closePropPL:"",
      closeBrokerPL:"",
      autoPrice:true,
      lastAutoClose:{
        ...
        status:"CALCOLATO_NON_CONFERMATO"
      }
    };
    await patchChallenge(row,nextState);
    return {challenge:row.challenge_id,status:`AUTO_${hit}`,alerts,balance:newBal};
  }
...
async function run(){
  ...
  const active=(Array.isArray(rows)?rows:[]).filter(r=>r?.state?.active);
  const results=[];
