...
function LoginForm() {
  ...
  const from = searchParams.get("from") || "/profit-tracker";
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/login-profit-tracker", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password }),
      });
      if (res.ok) {
        localStorage.setItem("profit_tracker_unlocked", "1");
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 28, fontWeight: 800, color: "#e8ecf5", marginBottom: 8 }}>
        PROFIT<span style={{ color: "#c8f135" }}>TRACKER</span>
      </div>
...
export default function LoginProfitTrackerPage() {
  return (
    <div style={{
      minHeight: "100vh",
      background: "#0d0f14",
